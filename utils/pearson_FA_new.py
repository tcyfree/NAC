...
# 假设 new_data 是新的数据集，格式与原始数据一致
new_data = pd.read_excel("./data/Post-NAT-BRCA/random_group_one_Post_TNBC_v1.1.xlsx")

new_data = new_data[selected_features_list]
...
# 2. 用原来的 StandardScaler 对新数据进行标准化
# 注意：这里不能用 fit_transform，而要使用 transform，这样新数据会按照原始数据的均值和标准差进行转换
new_data_scaled = scaler.transform(new_data_numeric)

# 3. 用已经拟合好的 FactorAnalyzer 进行因子得分的计算
new_factor_scores = fa.transform(new_data_numeric)  # 或 new_data_scaled, 取决于你的 FactorAnalyzer 是否内部做了标准化处理

# 4. 将因子得分转换为 DataFrame
new_factor_scores_df = pd.DataFrame(new_factor_scores, columns=[f"Factor {i+1} Score" for i in range(n_factors)])
new_factor_scores_df.index = new_data.index

# 输出或保存新的样本因子得分
print("新数据的因子得分：")
print(new_factor_scores_df.head())
new_factor_scores_df.to_excel("./data/IMPRESS/cluster/new_factor_scores_ml_pearson.xlsx", sheet_name="Factor Scores", index=True)
print("新数据的因子得分已保存到 './data/IMPRESS/cluster/new_factor_scores_ml_pearson.xlsx'")
